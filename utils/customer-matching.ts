...
export async function matchProfileWithCrm(
  ...
  try {
    ...
    if (profileError || !profile) {
      console.error('Error fetching profile:', profileError ? profileError.message : 'Profile not found for ID:', profileId);
      return null;
    }
  ...
  try {
    ...
    try {
      ...
      if (!recoveryMappingCheck) {
          ...
          await supabase.from('crm_customer_mapping_vip_staging').insert(placeholderDataOnError)
            .then(response => {
