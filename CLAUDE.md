...
CRON_API_KEY=             # shared secret for the review-request pg_cron job AND all internal self-fetch notify calls (/api/notifications/line + /line/review-request + /email + /email/review-request + /email/cancellation). Routes fail closed (401) if unset — see lib/internalAuth.ts
