...
async function sendOrderConfirmationEmail(
  ...
  const { data: order } = await supabase
    ...
    .select(
      'order_code, rental_subtotal, delivery_fee, total_price, delivery_requested, delivery_address, delivery_time, return_time, start_date, end_date, start_time, duration_days, customer_id, customer_name, customer_email',
    )
    .eq('id', orderId)
    .maybeSingle();
