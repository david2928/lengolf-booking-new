 ...
 * (`step` and `step_index` were registered on 2026-06-21).
 *
 ...
 * Re-entering a step (back-navigation, or an auth round-trip that replays an
 * earlier step) does not re-emit, so per-step event counts are usable directly
 * as drop-off denominators. Course-rental moved to these semantics on
 * 2026-07-25; a course_rental_step_viewed count spanning that date will step
 * down, and needs an annotation rather than a trend reading. Bay booking has
 * these semantics from its first release.
 *
 ...
 * a `useFlowPersistence` restore, or an auth-return deep link that promotes the
 * step in a later effect. Steps passed over while disabled are NOT backfilled:
 * the customer never saw them, so they must not appear in the funnel.
 */
