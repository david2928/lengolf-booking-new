...
export function RentalPriceSummaryBar({
  ...
  if (rentalPrice <= 0 || durationDays <= 0) return null;
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0 flex-1">
              <p className="text-xs text-gray-500 truncate">
                {durationDays}d rental
                {savings > 0 && (
                  <span className="ml-1 text-green-600">
                    (save ฿{format.number(savings)})
                  </span>
                )}
              </p>
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                <span className="text-xs text-gray-400">฿{format.number(rentalPrice)} rental</span>
                {deliveryFee > 0 && (
