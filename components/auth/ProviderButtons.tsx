...
export function ProviderButtons({
  ...
  /**
   * Two visual registers, because these buttons do two different jobs.
   *
   * `stacked` (the login page) — brand-filled. There, signing in IS the task;
   * the buttons are the primary action on an otherwise empty card and should
   * carry Facebook blue and LINE green.
   *
   * `compact` (inside the booking form) — brand-NEUTRAL, styled as siblings of
   * the contact inputs: same radius, same border, same height family. This is
   * the whole idea. In the form these are not calls to action, they are an
   * INPUT METHOD for the three fields directly beneath them. Rendering them as
   * three filled brand colours put three competing accents into a step whose
   * only accent should be the green Confirm button, and made an optional
   * shortcut louder than the required fields it sits above.
   *
   * Colour survives in the brand MARKS only, which is what people actually
   * recognise. Everything else is form furniture.
   */
  const base =
    layout === 'compact'
      ? 'flex h-11 w-full items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-3 text-sm font-medium text-gray-700 transition-colors hover:border-gray-300 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-1 disabled:opacity-50 disabled:cursor-wait'
      : 'flex w-full items-center justify-center rounded-lg px-4 py-3 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-wait';

  /** Brand fills apply to the login page only; in-form they are neutral. */
  const skin = (filled: string) => (layout === 'compact' ? '' : filled);
  ...
  /**
   * Stacked, "Continue with Google / Facebook / LINE" reads naturally — one
   * phrase per row. Side by side it does not: "Continue with" appears three
   * times across a single row, becomes the loudest text in the block, and
   * carries no information the heading above has not already given.
   *
   * So `compact` labels each button with the brand alone. Brands are proper
   * nouns and are NOT translated — Google, Facebook and LINE render in Latin
   * script in Thai, Japanese, Korean and Chinese interfaces alike — which is
   * why this is a literal rather than a fourth set of message keys. Slicing the
   * brand out of the translated phrase would not work either: in Thai it reads
   * "ดำเนินการต่อด้วย Google", with the brand at the end.
   *
   * The full translated phrase stays as `aria-label`, so a screen reader still
   * hears "Continue with Google" and not a bare brand name.
   */
    ...
    <div className={minHeightClass ?? ''}>
      ...
      {showGoogle && (
        <button
          ...
          className={`${base} ${skin('border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 focus-visible:ring-gray-400')}`}
        >
          {loadingProvider === 'google' ? (
            <Spinner />
          ) : (
            <>
              <svg className={`h-5 w-5 ${isCompact ? '' : 'mr-3'}`} viewBox="0 0 24 24" aria-hidden="true">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              {isCompact ? 'Google' : t('continueWithGoogle')}
            </>
          )}
        </button>
      )}
      ...
      {showFacebook && (
        <button
          ...
          className={`${base} ${skin('bg-[#1877F2] text-white hover:bg-[#166FE5] focus-visible:ring-[#1877F2]')}`}
        >
          {loadingProvider === 'facebook' ? (
            ...
            <>
              <FaFacebook
                className={`text-lg ${isCompact ? 'text-[#1877F2]' : 'mr-3'}`}
                aria-hidden="true"
              />
              {isCompact ? 'Facebook' : t('continueWithFacebook')}
            </>
          )}
        </button>
      )}
      ...
      {showLine && (
        <button
          ...
          className={`${base} ${skin('bg-[#00B900] text-white hover:bg-[#00A000] focus-visible:ring-[#00B900]')}`}
        >
          {loadingProvider === 'line' ? (
            ...
            <>
              <FaLine
                className={`text-lg ${isCompact ? 'text-[#06C755]' : 'mr-3'}`}
                aria-hidden="true"
              />
              {isCompact ? 'LINE' : t('continueWithLine')}
            </>
          )}
        </button>
      )}
