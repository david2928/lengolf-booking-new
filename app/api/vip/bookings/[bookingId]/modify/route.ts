...
export async function POST(request: NextRequest, context: ModifyRouteContext) {
  ...
  try {
    ...
    delete (newBookingSnapshotForHistory as any).profiles; // Remove joined data before history insert
    ...
    const notificationData: NotificationBookingData = {
      ...
      phone_number: (cancelledBooking.profiles as any)?.phone_number || null,
