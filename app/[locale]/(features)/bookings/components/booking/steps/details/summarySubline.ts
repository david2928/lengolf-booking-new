 ...
 *
 * That pairing is now unconditional, which it briefly was not. A "slot chip"
 * used to restate the date, the start time and the bay at the top of the
 * session sub-step, and the header was silenced there to stop the pair being
 * read twice. The chip is gone and the subline speaks on every screen, so
 * wherever this row appears the header above it names the date, the start time
 * and the bay — with no sub-step to except.
 *
