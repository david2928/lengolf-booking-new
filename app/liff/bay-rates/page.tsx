...
export default function BayRatesPage() {
  ...
  const initializePage = async () => {
    try {
      ...
      const liffId = process.env.NEXT_PUBLIC_LIFF_BAY_RATES_ID;
      if (!liffId || liffId === 'your-liff-id-here') {
        console.log('[Bay Rates] LIFF ID not configured - running without LIFF features');
        setViewState('ready');
        return;
      }

      await window.liff.init({ liffId }).catch((err) => {
        console.warn('[Bay Rates] LIFF init failed - continuing without LIFF features:', err);
        setViewState('ready');
        return;
      });
